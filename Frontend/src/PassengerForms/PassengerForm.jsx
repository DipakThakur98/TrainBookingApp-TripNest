...
const MASTER_KEY = "tripnest_master_passengers";
...
const PassengerForm = () => {
  ...
  useEffect(() => {
    ...
    // Load master list passengers from localStorage
    const stored = localStorage.getItem(MASTER_KEY);
    if (stored) setMasterPassengers(JSON.parse(stored));
  }, []);
  ...
  // Select a saved passenger from Master List
  const selectFromMaster = (masterP) => {
    const filled = {
      name: masterP.name,
      age: masterP.age,
      gender: masterP.gender || "",
      country: masterP.country || "IN",
      berth: masterP.berth || "",
    };
    if (pickerTargetIdx !== null) {
      // Replace specific passenger slot
      setPassengers((prev) => {
        const copy = [...prev];
        copy[pickerTargetIdx] = filled;
        return copy;
      });
    } else {
      // Add as new passenger
      setPassengers((prev) => [...prev, filled]);
    }
  ...
  const handleContinue = async () => {
    ...
    // Store booking data temporarily for later PDF generation
    const bookingData = {
      train: state?.train,
      passengers,
      contact,
    ...
    // Initiate Stripe Checkout session and redirect
    try {
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 mt-28 grid md:grid-cols-3 gap-6">
      ...
      <div className="md:col-span-2 bg-white shadow rounded p-6 space-y-4">
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            ...
            {masterPassengers.length > 0 && (
              <button
                onClick={() => { setPickerTargetIdx(null); setShowMasterPicker(true); }}
                className="flex items-center gap-1 text-sm text-orange-600 hover:text-orange-700 font-medium"
              >
                <FaUsers /> Add from Master List
              </button>
            )}
          </div>
          {passengers.map((p, idx) => (
            <div key={idx} className="grid grid-cols-6 gap-2 mb-3 items-center border p-2 rounded">
              <div className="col-span-2 flex gap-1">
                ...
                {masterPassengers.length > 0 && (
                  <button
                    onClick={() => { setPickerTargetIdx(idx); setShowMasterPicker(true); }}
                    className="text-orange-500 hover:text-orange-600 px-1"
                    title="Select from Master List"
                  >
                    <FaUsers size={14} />
                  </button>
                )}
        ...
        {/* Master List Picker Modal */}
        {showMasterPicker && (
          <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center">
            <div className="bg-white rounded-xl shadow-xl w-full max-w-md mx-4 p-6">
              <div className="flex items-center justify-between mb-4">
                <h3 className="font-bold text-lg">Select Passenger</h3>
                <button onClick={() => setShowMasterPicker(false)} className="text-gray-400 hover:text-gray-600">
                  <MdOutlineClose size={22} />
                </button>
              </div>
              {masterPassengers.length === 0 ? (
                <p className="text-gray-500 text-center py-6">No saved passengers. Go to Master List to add.</p>
              ) : (
                <div className="space-y-2 max-h-72 overflow-y-auto">
                  {masterPassengers.map((mp) => (
                    <button
                      key={mp.id}
                      onClick={() => selectFromMaster(mp)}
                      className="w-full text-left px-4 py-3 border rounded-lg hover:bg-orange-50 hover:border-orange-400 transition"
                    >
                      <p className="font-semibold text-gray-800">{mp.name}</p>
                      <p className="text-sm text-gray-500">
                        Age: {mp.age} • {mp.gender || 'N/A'} • Berth: {mp.berth || 'Any'}
                      </p>
                    </button>
                  ))}
                </div>
